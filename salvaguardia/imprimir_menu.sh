...
read -p "Ingresa el sufijo de la carpeta de etiquetas a imprimir: " seleccion
...
if [ -n "$subcarpetaSeleccionada" ]; then
    ...
    if [ ${#archivos[@]} -eq 0 ]; then
        echo "No se encontraron etiquetas en '$(basename "$subcarpetaSeleccionada")'."
    else
        echo "Etiquetas en '$(basename "$subcarpetaSeleccionada")':"
